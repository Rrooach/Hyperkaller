...
fs_image_segment_check(unsigned long size, unsigned long nsegs, long segments) {
  unsigned long i;
  // Strictly saying we ought to do a nonfailing copyout of segments into a
  // local var. But some filesystems have large number of segments (2000+), we
  // can't allocate that much on stack and allocating elsewhere is problematic,
...
// flags flags[mount_flags], opts ptr[in, fs_options[vfat_options]])
// fs_image_segment {
...
static long syz_mount_image(volatile long fsarg, volatile long dir,
  ...
  debug("syz_mount_image: size=%llu segs=%llu loop='%s' dir='%s' fs='%s' "
        "flags=%llu opts='%s'\n",
        (uint64)size, (uint64)nsegs, loopname, (char *)dir, fs, (uint64)flags,
        opts);
#if SYZ_EXECUTOR
  cover_reset(0);
#endif
...
static void checkpoint_iptables(struct ipt_table_desc *tables, int num_tables,
  ...
  for (i = 0; i < num_tables; i++) {
    ...
    if (getsockopt(fd, level, IPT_SO_GET_INFO, &table->info, &optlen)) {
      ...
      fail("iptable checkpoint %s/%d: getsockopt(IPT_SO_GET_INFO)", table->name,
           family);
    }
    ...
    if (table->info.size > sizeof(table->replace.entrytable))
      fail("iptable checkpoint %s/%d: table size is too large: %u", table->name,
           family, table->info.size);
    if (table->info.num_entries > XT_MAX_ENTRIES)
      fail("iptable checkpoint %s/%d: too many counters: %u", table->name,
           family, table->info.num_entries);
...
static void reset_iptables(struct ipt_table_desc *tables, int num_tables,
  ...
  for (i = 0; i < num_tables; i++) {
    ...
    if (memcmp(&table->info, &info, sizeof(table->info)) == 0) {
      ...
      if (getsockopt(fd, level, IPT_SO_GET_ENTRIES, &entries, &optlen))
        fail("iptable %s/%d: getsockopt(IPT_SO_GET_ENTRIES)", table->name,
             family);
    ...
    if (setsockopt(fd, level, IPT_SO_SET_REPLACE, &table->replace, optlen))
      fail("iptable %s/%d: setsockopt(IPT_SO_SET_REPLACE)", table->name,
           family);
  }
  close(fd);
}
...
static void checkpoint_arptables(void) {
  ...
  for (i = 0; i < sizeof(arpt_tables) / sizeof(arpt_tables[0]); i++) {
    ...
    if (table->info.size > sizeof(table->replace.entrytable))
      fail("arptable checkpoint %s: table size is too large: %u", table->name,
           table->info.size);
    if (table->info.num_entries > XT_MAX_ENTRIES)
      fail("arptable checkpoint %s: too many counters: %u", table->name,
    ...
    if (getsockopt(fd, SOL_IP, ARPT_SO_GET_ENTRIES, &entries, &optlen))
      fail("arptable checkpoint %s: getsockopt(ARPT_SO_GET_ENTRIES)",
           table->name);
    table->replace.valid_hooks = table->info.valid_hooks;
    table->replace.num_entries = table->info.num_entries;
    table->replace.size = table->info.size;
...
static void checkpoint_ebtables(void) {
  ...
  for (i = 0; i < sizeof(ebt_tables) / sizeof(ebt_tables[0]); i++) {
    ...
    if (getsockopt(fd, SOL_IP, EBT_SO_GET_INIT_INFO, &table->replace,
      ...
      fail("ebtable checkpoint %s: getsockopt(EBT_SO_GET_INIT_INFO)",
           table->name);
    }
    ...
    if (table->replace.entries_size > sizeof(table->entrytable))
      fail("ebtable checkpoint %s: table size is too large: %u", table->name,
           table->replace.entries_size);
    ...
    if (getsockopt(fd, SOL_IP, EBT_SO_GET_INIT_ENTRIES, &table->replace,
                   &optlen))
      fail("ebtable checkpoint %s: getsockopt(EBT_SO_GET_INIT_ENTRIES)",
           table->name);
  }
  close(fd);
}
...
static void syz_setcon(const char *context) {
  ...
  if (bytes_written != (ssize_t)strlen(context))
    fail("setcon: Could not write entire context.  Wrote %zi, expected %zu",
...
// - No dynamic memory allocation
// - Uses fail() instead of returning an error code
static int syz_getfilecon(const char *path, char *context,
                          size_t context_size) {
...
static void syz_setfilecon(const char *path, const char *context) {
  ...
  if (strcmp(context, new_context) != 0)
    fail("setfilecon: could not set context to %s, currently %s", context,
         new_context);
}
