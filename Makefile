...
tidy:
	...
	clang-tidy -quiet -header-filter=.* -checks=-*,misc-definitions-in-headers  \
		-extra-arg=-DGOOS_$(TARGETOS)=1 -extra-arg=-DGOARCH_$(TARGETARCH)=1 \
		executor/*.cc
	# Just check for compiler warnings. -warnings-as-errors=*
	$(CC) executor/test_executor.cc -c -o /dev/null #-Wparentheses  -Wall
